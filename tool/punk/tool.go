...
const (
	toolDoc = "https://github.com/bilibili/punk/blob/master/doc/wiki-cn/punk-tool.md"
)
...
func toolAction(c *cli.Context) (err error) {
	if c.NArg() == 0 {
		...
		fmt.Println("\n安装工具: punk tool install demo")
		fmt.Println("执行工具: punk tool demo")
		fmt.Println("安装全部工具: punk tool install all")
		fmt.Println("全部升级: punk tool upgrade all")
		fmt.Println("\n详细文档：", toolDoc)
		return
	}

...
var (
	...
	_selfDiscoveryFeatrue feature2.Feature = "discovery.self"
	_discoveryFeatures                     = map[feature2.Feature]feature2.Spec{
...
func init() {
	feature2.DefaultGate.Add(_discoveryFeatures)
}
...
func fixConfig(c *Config) {
	if len(c.Nodes) == 0 {
		c.Nodes = []string{"discovery.angrymiao.com"}
	}
	if env.Region != "" {
		c.Region = env.Region
	}
...
func New(c *Config) (d *Discovery) {
	if c == nil {
		c = &Config{
			Nodes:  []string{"discovery.angrymiao.com"},
			Key:    "discovery",
			Secret: "discovery",
		}
	}
	...
	if feature2.DefaultGate.Enabled(_selfDiscoveryFeatrue) {
...
func (d *Discovery) cancel(ins *naming.Instance) (err error) {
	...
	if ec := ecode.Int(res.Code); !ecode.EqualError(ec, ecode.OK) {
...
func (d *Discovery) renew(ctx context.Context, ins *naming.Instance) (err error) {
	...
	if ec := ecode.Int(res.Code); !ecode.EqualError(ec, ecode.OK) {
		err = ec
		if ecode.EqualError(ec, ecode.NothingFound) {
			return
		}
...
func (d *Discovery) polls(ctx context.Context, host string) (apps map[string]appData, err error) {
	...
	if ec := ecode.Int(res.Code); !ecode.EqualError(ec, ecode.OK) {
		if !ecode.EqualError(ec, ecode.NotModified) {
